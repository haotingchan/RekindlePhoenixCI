      ...
      (SELECT APDK_PARAM_KEY,APDK_KIND_ID2,APDK_EXPIRY_TYPE,APDK_PC_CODE,AI2_YMD AS APDK_YMD
       FROM
          (SELECT APDK_PARAM_KEY,APDK_KIND_ID2,APDK_EXPIRY_TYPE,APDK_PC_CODE
           FROM
              (SELECT APDK_PARAM_KEY,
               ...
               FROM CI.APDK
               WHERE APDK_PARAM_KEY IN ('RHO','RTO')
               AND APDK_KIND_ID2 LIKE :as_kind_id2
               GROUP BY APDK_PARAM_KEY,case when :as_expiry_type='9' then ' ' else APDK_KIND_ID2 end,
        ...
        (SELECT AA2_YMD,AA2_PARAM_KEY,
                ...
                SUM(AA2_AMT_STK_ORG) / 2 as AA2_AMT_STK_ORG
         FROM CI.AA2
         WHERE AA2_YMD >= :as_symd
         AND AA2_YMD <= :as_eymd
         AND AA2_PARAM_KEY IN ('RHO','RTO')
       ...
       --戶數AM9(全部期別999)
        (SELECT AM9_YMD,AM9_PARAM_KEY,
                AM9_KIND_ID2  as AM9_KIND_ID2, 
                ' ' as AM9_PC_CODE,
                AM9_ACC_CNT
         FROM CI.AM9
         WHERE AM9_YMD >= :as_symd
         AND AM9_YMD <= :as_eymd
         AND AM9_PARAM_KEY IN ('RHO','RTO')
       ...
       --成交筆數AM10
        (SELECT AM10_YMD,AM10_PARAM_KEY,
                ...
                SUM(AM10_QNTY),
                SUM(AM10_CNT) AS AM10_CNT FROM CI.AM10
         WHERE AM10_YMD >= :as_symd
         AND AM10_YMD <= :as_eymd
         AND AM10_PROD_TYPE = 'O'
         AND AM10_PARAM_KEY IN ('RHO','RTO')
