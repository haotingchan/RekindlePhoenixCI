...
namespace DataObjects.Dao.Together {
   public class D30591:DataGate {
      ...
      /// <returns></returns>
      public DataTable GetData(string as_expiry_type , string as_pc_code , string as_param_key , string as_kind_id2 , 
                               string as_symd , string as_eymd , string as_prod_type , string as_market_code) {
         object[] parms =
         {
...
SELECT APDK_YMD,
       ...
       AB4_ID_CNT
FROM
      (SELECT APDK_PARAM_KEY,APDK_KIND_ID2,APDK_EXPIRY_TYPE,APDK_PC_CODE,AI2_YMD AS APDK_YMD
       FROM
         (SELECT APDK_PARAM_KEY,APDK_KIND_ID2,APDK_EXPIRY_TYPE,APDK_PC_CODE
            FROM
               (SELECT APDK_PARAM_KEY,
                ...
                FROM CI.APDK
                WHERE APDK_PARAM_KEY = :as_param_key
      ...
      (SELECT AA2_YMD,AA2_PARAM_KEY,
              ...
              ' ' as AA2_PC_CODE,SUM(AA2_AMT)/2 AS AA2_AMT,
              SUM(AA2_AMT_STK)/2 AS AA2_AMT_STK
       FROM CI.AA2
       WHERE AA2_YMD >= :as_symd
       AND AA2_YMD <= :as_eymd
       AND AA2_PARAM_KEY = :as_param_key
       AND AA2_KIND_ID2 LIKE :as_kind_id2
       AND AA2_MARKET_CODE LIKE trim(:as_market_code)||'%'
       GROUP BY AA2_YMD,AA2_PARAM_KEY,case when :as_expiry_type='9' then ' ' else AA2_KIND_ID2 end),
       --ID數AB4(全部期別999
      (SELECT TO_CHAR(AB4_DATE,'YYYYMMDD') AS AB4_YMD,AB4_PARAM_KEY,
       ...
       AND AB4_PARAM_KEY = :as_param_key
      ...
      (SELECT AM9_YMD,AM9_PARAM_KEY,
       ...
       AND AM9_YMD <= :as_eymd
       AND AM9_PARAM_KEY = :as_param_key
      ...
      (SELECT AM10_YMD,
              ...
              SUM(AM10_QNTY),SUM(AM10_CNT) AS AM10_CNT FROM CI.AM10
       WHERE AM10_YMD >= :as_symd
       AND AM10_YMD <= :as_eymd
       AND AM10_PROD_TYPE = :as_prod_type
       AND AM10_PARAM_KEY = :as_param_key
