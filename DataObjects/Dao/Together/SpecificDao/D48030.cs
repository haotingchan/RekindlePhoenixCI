...
select mg1_kind_id,
    ...
    round(avg(round(mg1_cp_risk,4)),4) as avg_risk,
    max(round(mg1_cp_risk,4)) as max_risk,
    min(round(mg1_cp_risk,4)) as min_risk,
    sum(case when round(mg1_cp_risk,4) < (:ad_min_risk - :ad_range) then 1 else 0 end) as level_1,
    sum(case when round(mg1_cp_risk,4) >  (:ad_min_risk - :ad_range)   and round(mg1_cp_risk,4) <= :ad_min_risk then 1 else 0 end) as level_2,

    sum(case when round(mg1_cp_risk,4) >  :ad_min_risk and round(mg1_cp_risk,4) < (:ad_min_risk + :ad_range)  then 1 else 0 end) as level_3,
