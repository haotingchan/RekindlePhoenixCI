   ...
   public class D500xx
   {
      ...
      public string ConditionWhereSyntax()
      {
         ...
         if (!string.IsNullOrEmpty(Sdate)) {
            iswhere.Append($@" and {TableName}_YMD >='{CheckValue(Sdate)}' ");
         }
         if (!string.IsNullOrEmpty(Edate)) {
            iswhere.Append($@" and {TableName}_YMD <='{CheckValue(Edate)}' ");
         }
         /* 期貨商代號起迄 */
         if (!string.IsNullOrEmpty(Sbrkno)) {
            iswhere.Append($@" and {TableName}_BRK_NO >='{CheckValue(Sbrkno)}' ");
         }
         if (!string.IsNullOrEmpty(Ebrkno)) {
            iswhere.Append($@" and {TableName}_BRK_NO <='{CheckValue(Ebrkno)}' ");
         }
         ...
         *******************/
         ...
         if (!GbGroup.Equals("rb_gall")) {
            if (!string.IsNullOrEmpty(ProdCategory)) {
               iswhere.Append($@" and {TableName}_PARAM_KEY ='{CheckValue(ProdCategory)}' ");
            }
            /* 個股商品 */
            if (!GbGroup.Equals("rb_gparam")) {
               if (!string.IsNullOrEmpty(ProdKindIdSto)) {
                  iswhere.Append($@" and {TableName}_KIND_ID2 ='{CheckValue(ProdKindIdSto)}' ");
               }
               /* 商品 */
               if (!GbGroup.Equals("rb_gkind2")) {
                  if (!string.IsNullOrEmpty(ProdKindId)) {
                     iswhere.Append($@" and {TableName}_KIND_ID ='{CheckValue(ProdKindId)}' ");
                  }
