...
namespace DataObjects.Dao.Together
{
    ...
    public class LOGSP
    {
        ...
        public DataTable ListDataByMonth(DateTime LOGSP_DATE, string LOGSP_TXN_ID, LogspQueryType logspQueryType)
        {
            object[] parms =
            {
                "@LOGSP_DATE", LOGSP_DATE,
            ...
            string sql =
                @"
                    SELECT  * 
                    FROM    ci.LOGSP 
                    WHERE TO_CHAR(LOGSP_DATE,'YYYYMM') = TO_CHAR(@LOGSP_DATE ,'YYYYMM')
                    AND LOGSP_TXN_ID = @LOGSP_TXN_ID 
                ";
        ...
        public bool SaveByMonth(DateTime LOGSP_DATE, string LOGSP_TXN_ID, int LOGSP_SEQ_NO, string LOGSP_TID, string LOGSP_TID_NAME, DateTime LOGSP_BEGIN_TIME, DateTime LOGSP_END_TIME, string LOGSP_MSG)
        {
            object[] parms =
            {
                "@LOGSP_DATE", LOGSP_DATE,
            ...
            string sqlQuery =
                @"
                    SELECT * FROM CI.LOGSP WHERE TO_CHAR(LOGSP_DATE,'YYYYMM')=TO_CHAR(@LOGSP_DATE,'YYYYMM') AND LOGSP_TXN_ID=@LOGSP_TXN_ID AND LOGSP_TID=@LOGSP_TID
                 ";
            ...
            string sqlUpdate =
                @"
                    UPDATE CI.LOGSP 
	                SET LOGSP_DATE=@LOGSP_DATE,LOGSP_BEGIN_TIME=@LOGSP_BEGIN_TIME, LOGSP_END_TIME=@LOGSP_END_TIME, LOGSP_MSG=@LOGSP_MSG
	                WHERE TO_CHAR(LOGSP_DATE,'YYYYMM')=TO_CHAR(@LOGSP_DATE,'YYYYMM') AND LOGSP_TXN_ID=@LOGSP_TXN_ID AND LOGSP_TID=@LOGSP_TID
                 ";
