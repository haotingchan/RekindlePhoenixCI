...
namespace Common.Config {
   public class SettingDragons<T> where T : class {
      ...
      public DBInfo GetDBInfo(string dbSchema) {
         DBInfo resultInfo;

         switch (dbSchema) {
            case "ci":
               resultInfo = SettingDragons.Instance.Setting.Database.Ci;
               break;
            case "fut":
               resultInfo = SettingDragons.Instance.Setting.Database.CiFut;
               break;
            case "opt":
               resultInfo = SettingDragons.Instance.Setting.Database.CiOpt;
               break;
            case "futAH":
               resultInfo = SettingDragons.Instance.Setting.Database.CiFutAH;
               break;
            case "optAH":
               resultInfo = SettingDragons.Instance.Setting.Database.CiOptAH;
               break;
            case "pos_owner":
               resultInfo = SettingDragons.Instance.Setting.Database.POSOnwer;
               break;
            default:
               resultInfo = null;
               break;
