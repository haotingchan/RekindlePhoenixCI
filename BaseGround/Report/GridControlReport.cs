...
{
   public partial class GridControlReport : XtraReport
   {
      public GridControlReport()
      {
         InitializeComponent();
      }
      public GridControlReport(DataTable gridData, GridView gv)
      {
         InitializeComponent();
         ArrayList columnFileName = new ArrayList();
         ArrayList columnCation = new ArrayList();
         ...
         for (int i = 0; i < gv.Columns.Count; i++)
            if (gv.Columns[i].Visible && gv.Columns[i].GroupIndex < 0) {
               columnFileName.Add(gv.Columns[i].FieldName);
               columnCation.Add(gv.Columns[i].Caption);
            }
         InitTables(columnCation, columnFileName);
      }
      ...
      public void InitTables(ArrayList columns, ArrayList fileNames)
      {
         ...
         for (int i = 0; i < colCount; i++) {
            ...
            cell2.DataBindings.Add("Text", null, fileNames[i].ToString());
