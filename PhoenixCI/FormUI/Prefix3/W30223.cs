...
namespace PhoenixCI.FormUI.Prefix3 {
   ...
   public partial class W30223 : FormParent {
      ...
      protected void wf_30223_ch2(DataTable dt , Worksheet ws) {
         ...
         ws.MergeCells(ws.Range["A1:I1"]); //Merge回來，表頭才會置中
      ...
      protected void wf_30223_eng2(DataTable dt , Worksheet ws) {
         ...
         ws.MergeCells(ws.Range[string.Format("A{0}:J{1}" , rowIndex + 1 , rowIndex + 1)]); //Merge註解1

         rowIndex = rowIndex + 1;
         ws.Cells[rowIndex , 0].Value = ws.Cells[rowIndex , 0].Value.ToString() + lowerYmd + ".";
         ws.MergeCells(ws.Range[string.Format("A{0}:J{1}" , rowIndex + 1 , rowIndex + 2)]); //Merge註解2

         ws.MergeCells(ws.Range[string.Format("A{0}:J{1}" , rowIndex + 3 , rowIndex + 4)]); //Merge註解3
