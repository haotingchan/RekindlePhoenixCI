   ...
   public partial class W30723 : FormParent {
      ...
      protected override ResultStatus Export() {
         ...
         try {
            ...
            string dateMD = txtDate.DateTimeValue.ToString("yyyyMM");
            string asParamKey = "MXF";

            #region Get AM2 Data 若沒有AM2 Data連檔案都不複製
            dtAM2 = daoAm2.ListAm2DataByYmd(dateMD, asParamKey);

            if (dtAM2.Rows.Count <= 0) {
               ExportShow.Hide();
               MessageDisplay.Info(dateMD + "," + _ProgramID + '－' + _ProgramName + ",無任何資料!");
