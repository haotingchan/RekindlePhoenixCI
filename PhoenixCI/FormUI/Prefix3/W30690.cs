...
namespace PhoenixCI.FormUI.Prefix3 {
   ...
   public partial class W30690 : FormParent {
      ...
      private void wf_30690_high_low(Workbook workbook, string from_ymd, string to_ymd, string prodType) {
         ...
         ws.Cells[1, 2].Value = from_ymd.AsDateTime("yyyyMMdd").AddYears(1).ToString("yyyy0101");
      ...
      private void wf_30690_third(Workbook workbook, string em_aft_to_ymd) {
         ...
         if (dt.Rows.Count < emptyRowCount) {
            ws.Rows.Remove(dt.Rows.Count + rowIndex, emptyRowCount + rowIndex);
         }
      ...
      private void btnCopy_Click(object sender, EventArgs e) {
         GroupBox panNormal = (GroupBox)this.Controls.Find("panNormal", true).FirstOrDefault();

         foreach (Control c in panNormal.Controls) {
            if (c.Name.IndexOf("txt") >= 0) {
               TextDateEdit dateEditNormal = (TextDateEdit)c;
               TextDateEdit dateEditNight = (TextDateEdit)this.Controls.Find(c.Name + "N", true).FirstOrDefault();

               //將日盤查詢日期帶入夜盤
               dateEditNight.DateTimeValue = dateEditNormal.DateTimeValue;
            }
         }
      }
