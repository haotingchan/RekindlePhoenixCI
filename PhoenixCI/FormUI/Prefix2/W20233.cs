   ...
   public partial class W20233 : FormParent {
         ...
         //寫入資料庫
            ...
            foreach (DataRow dr in dt.Rows) {
               if (dr.RowState == DataRowState.Added) {
                  for (int w = 0 ; w < 3 ; w++) {
                     if (string.IsNullOrEmpty(dr[w].AsString())) {
                        MessageDisplay.Info("資料尚未填寫完成!" , GlobalInfo.ResultText);
      ...
      protected override ResultStatus InsertRow() {
         ...
         if (gcMain.DataSource == null) {
            DataTable returnTable = daoSTKOUT.ListAllByDate(txtDate.Text.Replace("/" , ""));
            returnTable.Columns.Add("Is_NewRow" , typeof(string));
            DataTable dtTmp = returnTable.Clone();
            gcMain.DataSource = dtTmp;
            gvMain.AddNewRow();
            gvMain.SetRowCellValue(GridControl.NewItemRowHandle , gvMain.Columns["STKOUT_DATE"] , txtDate.DateTimeValue.ToString("yyyyMMdd"));
