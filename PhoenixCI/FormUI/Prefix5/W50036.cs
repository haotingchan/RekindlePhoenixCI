...
namespace PhoenixCI.FormUI.Prefix5
{
   ...
   public partial class W50036 : FormParent
   {
      ...
      protected bool GetData()
      {
         ...
         //交易時段選盤後
         if (gbMarket.EditValue.Equals("rb_market_1")) {
            _Data = dao50036.ListAH(_D500Xx);
         }
         else if (gbDetial.EditValue.Equals("rb_gdate")) {
            _Data = dao50036.List50036(_D500Xx);//報表內容選擇分日期
         }
      ...
      protected override ResultStatus Retrieve()
      {
         ...
         List<ReportProp> caption = new List<ReportProp>{
            ...
            new ReportProp{DataColumn = "AMM0_MMK_RATE",Caption="有效報/詢價比例",CellWidth=75,textAlignment=TextAlignment.MiddleRight,TextFormatString="{0:##0.0#}"},
            new ReportProp{DataColumn = "AMM0_KEEP_TIME",Caption="每日平均維持時間(分)",CellWidth=80,textAlignment=TextAlignment.MiddleRight,TextFormatString="{0:#}"},
            new ReportProp{DataColumn = "AMM0_RESULT",Caption="績效分數",CellWidth=150,textAlignment=TextAlignment.MiddleRight,TextFormatString="{0:##0.#}"},
      ...
      protected override ResultStatus Export()
      {
         ...
         try {
            ...
            string lsFilename = _ProgramID + "_" + DateTime.Now.ToString("yyyy.MM.dd") + "-" + DateTime.Now.ToString("HH.mm.ss");
            _D500Xx.Filename = Path.Combine(GlobalInfo.DEFAULT_REPORT_DIRECTORY_PATH, lsFilename + ".xlsx");
            if (_D500Xx.Filename == "") {
               return ResultStatus.Fail;
            }
            workbook.SaveDocument(_D500Xx.Filename, DocumentFormat.Xlsx);//新增要寫入的Excel
            _D500Xx.LogText = _D500Xx.Filename;
            DataTable dt = _Data.Copy();

            dt.Columns["AMM0_YMD"].ColumnName = "日期";
            dt.Columns["AMM0_BRK_NO"].ColumnName = "期貨商代號";
            dt.Columns["BRK_ABBR_NAME"].ColumnName = "期貨商名稱";
            dt.Columns["AMM0_ACC_NO"].ColumnName = "投資人帳號";
            dt.Columns["AMM0_PROD_ID"].ColumnName = "商品名稱";
            dt.Columns["AMM0_MMK_QNTY"].ColumnName = "造市量";
            dt.Columns["AMM0_TOT_QNTY"].ColumnName = "造市者總成交量";
            dt.Columns["AMM0_MMK_RATE"].ColumnName = "有效報/詢價比例";
            dt.Columns["AMM0_KEEP_TIME"].ColumnName = "每日平均維持時間(分)";
            dt.Columns["AMM0_RESULT"].ColumnName = "績效分數";
            dt.Columns["AMM0_CONTRACT_CNT"].ColumnName = "合格檔數";
            dt.Columns.Remove(dt.Columns["CP_ROW"]);
            /******************
            開啟檔案
            ******************/
