   ...
   public partial class W40210 : FormParent {
      ...
      protected override ResultStatus Export() {
         try {
            ...
            if (cbxProd.Items[0].CheckState == System.Windows.Forms.CheckState.Checked
               ...
               //3.1 現貨data
               wf_40210_3(workbook, StartDate, EndDate, listCode, TotalDayCount);
      ...
      protected ResultStatus wf_40210_1(Workbook workbook, string kind, string is_symd, string is_eymd, decimal chi_150, decimal chi_180, decimal v365, int TotalDayCount) {
         try {
            ...
            if (dtTarget.Rows.Count <= 0) {
               MessageDisplay.Info(string.Format("{0},{1}－{2},讀取無任何資料!", is_eymd, reportId, reportName));
               return ResultStatus.Fail;
            }
            ...
            if (dtDetail.Rows.Count <= 0) {
               MessageDisplay.Info(string.Format("{0}-{1},{2}－{3}明細,讀取無任何資料!", is_symd, is_eymd, reportId, reportName));
               return ResultStatus.Fail;
            }
      ...
      protected ResultStatus wf_40210_3(Workbook workbook, string is_symd, string is_eymd, List<string> is_code, int TotalDayCount) {
         try {
            ...
            if (dtTarget.Rows.Count <= 0) {
               MessageDisplay.Info(string.Format("{0}-{1},{2}－{3},讀取無任何資料!", is_symd, is_eymd, reportId, reportName));
               return ResultStatus.Fail;
            }
      ...
      protected ResultStatus wf_40210_8(Workbook workbook, string is_eymd, List<string> is_code) {
         try {
            ...
            if (dtDetail.Rows.Count <= 0) {
               MessageDisplay.Info(string.Format("{0},{1}－{2}明細,讀取無任何資料!", is_eymd, reportId, reportName));
               return ResultStatus.Fail;
            }
