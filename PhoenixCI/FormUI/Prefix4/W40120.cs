    ...
    public partial class W40120 : FormParent {
        ...
        protected override ResultStatus Export() {
            try {
                ...
                if (dt.Rows.Count <= 0) {
                    MessageDisplay.Info(string.Format("{0},{1}文字說明,讀取「案由一契約名稱」無任何資料!", txtStartDate.Text, _ProgramID));
                ...
                if (dtFuture.Rows.Count > 0) {
                    ...
                    Decimal mg1_change_cond = Math.Round(dtFuture.Rows[dtFuture.Rows.Count - 1]["mg1_change_cond"].AsDecimal() * 100, 0);
                ...
                if (dtOption.Rows.Count > 0) {
                    ...
                    Decimal mg1_change_cond = Math.Round(dtOption.Rows[dtOption.Rows.Count - 1]["mg1_change_cond"].AsDecimal() * 100, 0);
                    ls_txt_end = string.Format("其變動幅分達{0}%以上且進位後金額改變，依本公司結算保證金收取方式及標準規定，已達得調整保證金之標準。",
                                                    mg1_change_cond.ToString());
