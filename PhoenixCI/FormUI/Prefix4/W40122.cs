    ...
    public partial class W40122 : FormParent {
        ...
        protected override ResultStatus Export() {
            try {
                ...
                if (dt.Rows.Count <= 0) {
                    MessageDisplay.Info(string.Format("{0},{1},讀取「SPAN參數調整資料」無任何資料!", txtStartDate.Text, this.Text));
                ...
                if (dtSV.Rows.Count > 0) {
                    ...
                    for (int k = 0;k < dtSV.Rows.Count;k++) {
                        ...
                        Decimal changeRange = Math.Round(dtSV.Rows[k]["sp1_change_range"].AsDecimal() * 100, 1);
                        resSV += kindIdOut + "：" + changeRange + "%" + ConnString(k, dtSV.Rows.Count);
                    }//for (int k = 0;k < dtSV.Rows.Count;k++) {

                    string temp = new D40122().GetRate("SV");
                    decimal rate = Math.Round(temp.AsDecimal(0) * 100, 0);
                ...
                if (dtSD.Rows.Count > 0) {
                    ...
                    for (int k = 0;k < dtSD.Rows.Count;k++) {
                        Decimal changeRange = Math.Round(dtSD.Rows[k]["sp1_change_range"].AsDecimal() * 100, 1);
                ...
                if (dtSS.Rows.Count > 0) {
                    ...
                    for (int k = 0;k < dtSS.Rows.Count;k++) {
                        Decimal changeRange = Math.Round(dtSS.Rows[k]["sp1_change_range"].AsDecimal() * 100, 1);
