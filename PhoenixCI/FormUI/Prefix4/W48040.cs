   ...
   public partial class W48040 : FormParent {
      ...
      protected override ResultStatus Export() {
         ...
         try {
            ...
            DateTime startDate = dtDate.Rows[dtDate.Rows.Count - 1]["SDATE"].AsDateTime();
            DateTime endDate = dtDate.Rows[dtDate.Rows.Count - 1]["EDATE"].AsDateTime();
            ...
            foreach (DataRow drKind in dtTemp.Rows) {
               ...
               string excelDestinationPath = CopyExcelTemplateFile2(_ProgramID , kindId);
               ...
               //2.4讀取子table data (mg1_date/mg1_risk/mg1_min_risk)
               DataTable dtSingleKind = dao48040.ListKindByKindId(kindId , startDate , endDate);
               if (dtSingleKind.Rows.Count <= 0) {
                  workbook.SaveDocument(excelDestinationPath);//存檔
                  labMsg.Text = string.Format("{0},{1}~{2}無任何資料!" , kindId , startDate , endDate);
                  this.Refresh();
                  continue;
               }
               ...
               foreach (DataRow drDate in dtDate.Rows) {
                  ...
                  DataView dvSingleTemp = new DataView(dtSingleKind , "" , "mg1_date" , DataViewRowState.CurrentRows);
      ...
      protected string CopyExcelTemplateFile2(string fileName , string kindId = "") {
         ...
         if (kindId != "") {
            tempFileName += string.Format("({0})" , kindId);
         }

         string destinationFilePath = Path.Combine(GlobalInfo.DEFAULT_REPORT_DIRECTORY_PATH ,
                                     tempFileName + "_" + DateTime.Now.ToString("yyyy.MM.dd-HH.mm.ss") + ".xlsx");
