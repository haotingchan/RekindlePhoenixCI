...
namespace PhoenixCI.BusinessLogic.Prefix4
{
   ...
   public class B4001xTemplate : I4001x
   {
      ...
      /// </summary>
      /// <returns></returns>
      public string ComputeEWMA(string FilePath)
      {
         Workbook workbook = new Workbook();
         try {
            //切換Sheet
            ...
            //1.RowData sheet第一行
            DataTable dtCPR = dao.List40010CPR(emdate, "40010_1");
            //確認有無資料
            ...
            foreach (DataRow dr in dtCPR.Rows) {
               ...
               worksheet.Cells["I1"].SetValue(dr["CPR_EFFECTIVE_DATE"]);
            }

            //2.寫入40010template計算EWMA
            DataTable RowData = dao.ListRowDataSheet(emdate);
   ...
   public interface I4001x
   {
      ...
      /// </summary>
      /// <returns></returns>
      string ComputeEWMA(string globalPath);
